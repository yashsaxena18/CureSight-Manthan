...
import { 
  ...
  MapPin, Video, Building, AlertCircle,
...
export default function BookAppointment() {
  ...
  const [appointmentMode, setAppointmentMode] = useState<'online' | 'clinic' | 'home-visit'>('clinic');
  ...
  const fetchDoctorDetails = async () => {
    ...
    try {
      ...
      // 🔧 FIXED: Try multiple API endpoints to find doctor profile
      const apiEndpoints = [
        `${API_BASE}/appointments/doctor/profile/${doctorId}`, // Route from appointment.js
        `${API_BASE}/doctor/doctor-profile/${doctorId}`,       // Route from doctorRoutes.js  
        `${API_BASE}/doctor/profile/${doctorId}`,              // Alternative route
        `${API_BASE}/doctors/${doctorId}`,                     // Alternative route
      ];
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (data.success && data.doctor) {
              ...
              break;
            } else if (data.doctor) { // Some APIs might not have success flag
              doctorData = data.doctor;
              successEndpoint = endpoint;
              break;
            }
      ...
      if (doctorData) {
        console.log('✅ Doctor data found via:', successEndpoint);
        ...
        const mappedDoctor: Doctor = {
          ...
          experience: doctorData.experience || doctorData.yearsOfExperience || 5,
          consultationFee: doctorData.consultationFee || 500,
          hospitalAffiliation: doctorData.hospitalAffiliation || 'Hospital',
          address: doctorData.address,
          city: doctorData.city,
          state: doctorData.state,
          rating: doctorData.rating?.average || doctorData.rating || 4.5,
          isVerified: doctorData.isVerified || false,
          availableSlots: doctorData.availableSlots || timeSlots
        };

        setDoctor(mappedDoctor);
      } else {
        throw new Error('Doctor not found in any endpoint');
      }
  ...
  const handleBookAppointment = async () => {
    ...
    try {
      ...
      const appointmentData = {
        ...
        paymentMethod,
        clinicDetails: appointmentMode === 'clinic' ? {
      ...
      if (response.ok && data.success) {
        toast({
          ...
          description: data.message || "Your appointment has been scheduled successfully."
        });
        ...
        <Card>
          <CardContent className="pt-16 pb-16 text-center">
            ...
            <div className="space-y-2">
              <Button onClick={() => fetchDoctorDetails()} variant="outline">
                Try Again
              </Button>
        ...
        <Card className="bg-blue-50 border-blue-200">
          <CardContent className="pt-4">
            <p className="text-xs text-blue-700">
              Debug: Doctor ID = {doctorId}, Doctor Found = {doctor ? 'Yes' : 'No'}
            </p>
          </CardContent>
        </Card>
      )}
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Select value={appointmentMode} onValueChange={(value: any) => setAppointmentMode(value)}>
                  <SelectTrigger>
          ...
          <Card>
            <CardContent className="pt-6">
              <Button 
                ...
                disabled={booking || !selectedDate || !selectedTime || !symptoms.trim()}
                className="w-full"
                size="lg"
              >
                {booking ? (
